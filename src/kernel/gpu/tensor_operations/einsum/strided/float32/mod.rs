...
pub fn forward<'a, const N_a: usize, const N_b: usize, const N_c: usize>(
                                    tensor_a: &Tensor<'a, GPU, Strided<N_a>, f32, N_a>, dim_a: usize,
                                    tensor_b: &Tensor<'a, GPU, Strided<N_b>, f32, N_b>, dim_b: usize,
                                    ) -> Tensor<'a, GPU, Strided<N_c>, f32, N_c> {
        //Check if dimensions are valid
        assert!(dim_a < N_a, "Invalid dimension");
        assert!(dim_b < N_b, "Invalid dimension");
        ...
        //Check if tensor shapes are compatible for operation
        assert!(tensor_a_shape[dim_a] != tensor_b_shape[dim_b], "Tensor shape mismatch");
